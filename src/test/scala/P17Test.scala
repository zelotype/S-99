...
class P17Test extends FunSuite with Matchers{
  ...
  test("return 2 list that split from the length was given -- version 2") {
    P17.splitList(3, List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)) shouldEqual (List('a, 'b, 'c),List('d, 'e, 'f, 'g, 'h, 'i, 'j, 'k))
  }
}
